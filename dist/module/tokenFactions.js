import { defaultColors, dispositionKey, TOKEN_FACTIONS_FLAGS, TOKEN_FACTIONS_FRAME_STYLE } from "./Hooks.js";
...
export class TokenFactions {
    ...
    static async updateTokenFaction(tokenData) {
        let tokens;
        try {
    ...
    static updateTokenInternal(token) {
        //@ts-ignore
        if (token instanceof Token && token.icon && bevelTexture && bevelTexture.baseTexture) {
            ...
            TokenFactions.drawBorderFaction(token, token.factionFrame);
            //@ts-ignore
            token.icon.zIndex = token.factionFrame.zIndex + 1;
        }
    }
    ...
    static drawBorder(token, borderColor, factionBorder) {
        const t = getGame().settings.get(TOKEN_FACTIONS_MODULE_NAME, 'borderWidth') || CONFIG.Canvas.objectBorderThickness;
        const baseOpacity = getGame().settings.get(TOKEN_FACTIONS_MODULE_NAME, 'base-opacity');
